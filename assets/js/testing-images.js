import { ebCheckForPage } from './utilities'
...
function ebTestingImages () {
  const images = document.querySelectorAll('img[data-src], img[src]')

  // Check if we're using remote media
  let remoteMediaMode = false
  if (settings['remote-media']?.development && settings['remote-media']?.development.length > 0) {
    remoteMediaMode = true
  }

  // Check whether there is a testing URL in settings
  let testingMode = false
  if (settings['remote-media']?.testing && settings['remote-media']?.testing.length > 0) {
    testingMode = true
  }
  ...
  if (images && testingMode) {
    ...
    images.forEach(function (image) {
      ...
      if (imageSrc) {
        let imageAtTestingURL = null
        if (remoteMediaMode) {
          // is remote
          imageAtTestingURL = imageSrc.replace(settings['remote-media']?.development, settings['remote-media']?.testing)
        } else {
          const bookFolder = document.querySelector('.wrapper').getAttribute('data-book-directory')
          imageAtTestingURL = settings['remote-media']?.testing + '/' + bookFolder + '/' + imageSrc
        }
...
export default ebTestingImages
